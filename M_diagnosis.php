...
<body>
    ...
    if ($do == "select") {
        $patients = all('mohamed_diagnosis');
    ...
    ?>
        ...
        <p>
            ...
            <a href="logout.php" class="btn btn-danger mx-2 my-3">Logout</a>
            <a href=" M_diagnosis.php?do=add " class="btn btn-primary">Create Diagnosis</a>
            <table class="table">
                <thead>
                    <tr>
                        <th scope="col">#</th>
                ...
                <tbody>
                    <?php foreach ($patients as $patient) { ?>
                        <tr>
                            ...
                            <td>
                                <a href="M_diagnosis.php?do=edit&id= <?= $patient['id']; ?>" class="btn btn-primary ">Edit</a>
                                <a href="M_diagnosis.php?do=delete&id= <?= $patient['id']; ?>" class="btn btn-danger my-2">Delete</a>
    ...
    ?>
        <div class="container my-5">
            ...
            <form action="M_diagnosis.php?do=insert" method="post" class="col-8 shadow mx-auto p-5 " enctype="multipart/form-data">
                <div class="form-floating mb-3">
                    <p><label>Select Patient:</label></p>
                    <?php $patients = all('mohamed_patients');
                    ?>
    ...
    <?php
        ...
        if (isset($errors)) {
            ...
            header("Refresh:3;url=M_diagnosis.php?do=add");

            echo "</div>";
        } else {
            $columns = "patient, severity, general_diagnosis, detailed_diagnosis";
            $values = "'$patient', '$severity', '$general_diagnosis', '$detaild_diagnosis'";
            insert("mohamed_diagnosis", $columns, $values);
            header("location:M_diagnosis.php");
        }
    } elseif ($do == "delete") {
        $id = $_GET['id'];
        delete("mohamed_diagnosis", $id);
        header("location:M_diagnosis.php");
    } elseif ($do == 'edit') {
        $id = $_GET['id'];
        $inpatient = single("mohamed_diagnosis", $id);
    ?>
        <div class="container my-5">
            <form action="M_diagnosis.php?do=update" method="post" class="col-8 shadow mx-auto p-5 " enctype="multipart/form-data">
                <input type="hidden" name="id" value="<?= $id ?>">
                <div class="form-floating mb-3">
                    <p><label>Select Patient:</label></p>
                    <?php $patients = all('ahmed_patients'); ?>
