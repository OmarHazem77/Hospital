...
<body>
    ...
    if ($do == "select") {
        $patients = all('mohamed_patients');

    ?>

        <div class="container my-5 ">
            <p>
            <h1>Welcome Dr Mohamed</h1>
            </p>
            <p>
            <h3>Your Patients</h3>
            </p>
            <a href="M_diagnosis.php" class="btn btn-success mx-2">Diagnosis</a>
            <a href=" M_patients.php?do=add " class="btn btn-primary">Create User</a>
            <a href="logout.php" class="btn btn-danger my-5 mx-2">Logout</a>
            <table class=" table ">
                <thead>
                    <tr>
                        <th scope="col">#</th>
                ...
                <tbody>
                    <?php foreach ($patients as $patient) { ?>
                        <tr>
                            ...
                            <td>
                                <a href="M_patients.php?do=edit&id= <?= $patient['id']; ?>" class="btn btn-success ">Edit</a>
                                <a href="M_patients.php?do=delete&id= <?= $patient['id']; ?>" class="btn btn-danger ">Delete</a>
            ...
            <div class="link translate-middle">
                <a href="logout.php" class="btn btn-danger translate-middle ">Logout</a>
                <a href=" M_patients.php?do=add " class="btn btn-success">Create User</a>
            </div>
        </div>
    ...
    ?>
        <div class="container my-5">
            ...
            <form action="M_patients.php?do=insert" method="post" class="col-8 shadow mx-auto p-5">
    ...
    <?php
        ...
        if (isset($errors)) {
            ...
            header("Refresh:3;url=M_patients.php?do=add");

            echo "</div>";
        } else {
            $columns = "name, phone, email, address, house_number";
            $values = "'$name', '$phone', '$email', '$address', '$house_number'";
            insert("mohamed_patients", $columns, $values);
            header("location:M_patients.php");
        }
    } elseif ($do == "delete") {
        $id = $_GET['id'];
        delete("mohamed_patients", $id);
        header("location:M_patients.php");
    } elseif ($do == 'edit') {
        $id = $_GET['id'];
        $patient = single("mohamed_patients", $id);
        // print_r($user);
    ?>

        <div class="container my-5">
            <form action="M_patients.php?do=update" method="post" class="col-8 shadow mx-auto p-5">
    ...
    <?php
        ...
        if (isset($errors)) {
            ...
            header("Refresh:3;url=M_patients.php");

            echo "</div>";
        } else {


            update("mohamed_patients", " name = '$name' , phone = '$phone' , email = '$email' , address = '$address' , house_number = '$house_number' ", $id);
            header("location:M_patients.php");
        }
    }
