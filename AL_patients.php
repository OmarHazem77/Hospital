...
<body>
    ...
    ?>
        ...
        <div class="container my-5">
            ...
            <a href="AL_diagnosis.php" class="btn btn-success mx-2 my-3">Diagnosis</a>
                ...
                <tbody>
                    <?php foreach ($patients as $patient) { ?>
                        <tr>
                            ...
                            <td>
                                <a href="AL_patients.php?do=edit&id= <?= $patient['id']; ?>" class="btn btn-primary ">Edit</a>
                                <a href="AL_patients.php?do=delete&id= <?= $patient['id']; ?>" class="btn btn-danger ">Delete</a>
    ...
    ?>
        <div class="container my-5">
            ...
            <form action="AL_patients.php?do=insert" method="post" class="col-8 shadow mx-auto p-5">
    ...
    <?php
        ...
        if (isset($errors)) {
            ...
            header("Refresh:3;url=AL_patients.php?do=add");

            echo "</div>";
        } else {
            $columns = "name, phone, email, address, house_number";
            $values = "'$name', '$phone', '$email', '$address', '$house_number'";
            insert("ali_patients", $columns, $values);
            header("location:AL_patients.php");
        }
    } elseif ($do == "delete") {
        $id = $_GET['id'];
        delete("ali_patients", $id);
        header("location:AL_patients.php");
    } elseif ($do == 'edit') {
        $id = $_GET['id'];
        $patient = single("ali_patients", $id);
        // print_r($user);
    ?>

        <div class="container my-5">
            <form action="AL_patients.php?do=update" method="post" class="col-8 shadow mx-auto p-5">
    ...
    <?php
        ...
        if (isset($errors)) {
            ...
            header("Refresh:3;url=AL_patients.php");

            echo "</div>";
        } else {


            update("ali_patients", " name = '$name' , phone = '$phone' , email = '$email' , address = '$address' , house_number = '$house_number' ", $id);
            header("location:AL_patients.php");
        }
    }
